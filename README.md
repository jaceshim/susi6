...
|---|---|
| 대학 | `서강대학교` → `서강대` (`대학교$` → `대`) |
| 전형 | 두 자료의 표기 차이를 걷어낸 뒤 견준다. 경쟁률 자료는 `서울캠퍼스 학생부교과(지역균형전형)`·`[정원내] 학생부종합(일반학생전형)` 처럼 적고, 입결 자료는 `교과(지역균형)`·`종합(일반)` 으로 적는다. **캠퍼스 접두어·정원내외·괄호·`전형`·반영비율 설명(`[교과 100%]`)** 을 지우고 `학생부교과→교과`, `학생부종합→종합` 으로 맞춘다. `교과성적우수자` 처럼 종류 표시가 이름의 일부인 경우가 있어, 종류를 뗀 알맹이와 붙은 원형을 모두 견준다. 순서 표시는 `Ⅰ·I·1` 을 한 글자로 모으고, **숫자가 살아 있는 짝을 먼저** 찾은 뒤에야 숫자를 떼고 견준다 — 그러지 않으면 `종합Ⅰ` 과 `종합Ⅱ` 가 한 이름이 되어 서로의 컷을 물고 온다 |
| 전형 (줄인 이름) | 입결이 긴 이름을 `기본-갈래` 로 줄여 적는 경우를 잇는다 — 경쟁률 `잠재능력우수자면접` ↔ 입결 `잠재능력-면접`. 토막이 **모두** 순서대로 들어 있고 첫 토막이 앞에서 시작할 때만 붙인다. 하나라도 빠뜨리면 `잠재능력-서류` 가 `…면접전형` 에 달라붙는다 |
