...
.github/workflows/ratio.yml     5분마다 수집 → 커밋
```
...
허브가 러너에서 막히므로 목록은 미리 뽑아 `data/univs.json` 에 넣어 두었습니다. **173개교**
(진학사 85 · 유웨이 88), 지역·접수기간·경쟁률 URL 포함. 그대로 push 하면 됩니다.

수집기는 매번 허브를 먼저 시도하고, 막혀 있으면 이 파일을 씁니다. 목록이 바뀌면
(대학 추가·마감일 변경) 브라우저에서 다시 뽑아 덮어쓰면 됩니다.
...
3. 다운로드된 `univs.json` 을 `data/univs.json` 로 교체

이 상태로 워크플로를 돌리면 **유웨이 88개교**가 5분마다 수집됩니다.
진학사 85개교는 **대학 검색 목록에는 그대로 보이지만** "수집 불가" 로 표시되고, 고르면 사유와
원본 경쟁률 페이지 링크가 뜹니다. 카드로 담는 것만 안 됩니다.

### 대학 검색

`<datalist>` 는 한글 IME 조합 중에 후보를 갱신하지 않아(예: "가톨" 을 입력하는 중에는 아무것도 안 뜸)
직접 만든 목록으로 바꿨습니다.

- 조합 중에도 후보가 갱신됩니다 (`compositionupdate`)
- **초성 검색**: `ㄱㅎ` → 경희대학교, `ㅅㅇㅅㄹ` → 서울시립대학교
- 조합 중 마지막 글자가 미완성이면(가톨 → 가토) 한 글자를 떼고 다시 찾아 후보가 끊기지 않습니다
- 위/아래 방향키 + Enter, Esc 로 닫기, 마우스 클릭 모두 동작
- 수집되지 않는 대학도 목록에 표시하고 사유를 알려줍니다
...
`안전한 접속 확인` 은 두 가지일 수 있어서 한 번 열어보고 판정하면 안 됩니다.

| 종류 | 성격 | 대응 |
|---|---|---|
| JS 챌린지 인터스티셜 | 잠깐 기다리면 쿠키가 발급되고 본문이 나옴 | 기다렸다 재시도하면 수집 가능 |
| 짧은 시간에 요청 과다 | 속도 제한. 잠시 뒤 풀림 | 간격을 늘리면 수집 가능 |
| IP 기반 하드 차단 | 아무리 기다려도 그대로 | 실행 위치를 옮겨야 함 |

처음 구현은 `domcontentloaded` 직후 본문을 읽어 **인터스티셜을 곧바로 하드 차단으로 오인**했고,
요청 간격이 120ms(85개를 약 10초에 훑음)여서 속도 제한을 자기가 유발할 수도 있었습니다. 지금은:

- 인터스티셜이 감지되면 **최대 12초 걷히기를 기다리고**, 5초 → 10초 백오프로 **3번까지 재시도**
- 요청 간격 기본 **800ms + 무작위 0~400ms**(`GAP_MS` 로 조절), 10개마다 1.2초 추가 휴식
- 개별 페이지도 403/429 면 4초 → 8초 간격으로 3회까지 재시도
- 연속 3회 막히면 그 계열은 더 두드리지 않고 남은 대학을 미수집으로 남김
- 챌린지를 통과한 쿠키를 `.cache/state.json` 에 저장해 다음 실행에서 재사용 (Actions 캐시로 이어짐)

`data/probe.json` 의 `verdict` 가 위 표의 어느 경우인지 알려줍니다
(`selfCleared`, `retryAfterWait`, `challenge.scripts`, `cookies` 도 함께 기록됩니다).
**수동 실행(Run workflow)할 때만** 진단이 돌아 5분 주기를 잡아먹지 않습니다.

요청을 더 천천히 보내려면 저장소 `Settings → Secrets and variables → Actions → Variables` 에
`GAP_MS` 를 `1500` 같은 값으로 추가하면 됩니다.
...
- 수집기는 **5분마다** 돌고, 페이지의 자동 갱신 주기는 1·3·5·10·30분에서 고를 수 있습니다.
  GitHub 스케줄러는 best-effort 라 혼잡하면 몇 분 늦게 돌 수 있습니다.
- 원본 갱신이 10분~1시간이라 5분 수집의 대부분은 같은 값입니다. 대신 원본이 바뀐 직후에
  더 빨리 반영됩니다. 같은 기준시각은 스냅샷을 덮어써서 이력이 부풀지 않습니다.
